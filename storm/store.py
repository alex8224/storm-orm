...
class Store(object):
    ...
    def raw_execute(self, statement, params=()):
        return self._connection.raw_execute(statement, params)
...
class ResultSet(object):
    ...
    def get_select_sql(self):
        state = State()
        statement = self._store._connection.compile(self._get_select(), state)
        params = state.parameters
        statement = convert_param_marks(statement, "?", "?")
        return statement, [param.get() for param in params]

    def fetchall(self, rowhandler=lambda row: row):
        sql, param = self.get_select_sql()
        conn = self._store._connection
        cursor = conn.raw_execute(sql, param)
        try:
            while 1:
                row = cursor.fetchmany()
                if row:
                    yield rowhandler(row[0])
                else:
                    break
        except:
            raise
    ...
    def fetchone(self, rowhandler=lambda row: row):
        sql, param = self.get_select_sql()
        conn = self._store._connection
        cursor = conn.raw_execute(sql, param)
        try:
            return rowhandler(cursor.fetchone())
        except:
            raise
        finally:
            cursor.close()
